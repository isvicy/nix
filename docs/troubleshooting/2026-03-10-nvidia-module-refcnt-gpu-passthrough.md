...
### 6. 根因确认：i2c_dev 模块 + OpenRGB

`i2c_dev` 模块本身在 attach adapter 时**不会**增加引用计数。引用发生在**用户态进程打开 `/dev/i2c-N` 设备文件时**：`i2cdev_open()` → `i2c_get_adapter()` → `try_module_get(adapter->owner)`，增加 adapter 所属模块（nvidia）的 refcnt。

OpenRGB 以 `Restart=always` 运行，持续打开所有 7 个 nvidia I2C adapter 对应的 `/dev/i2c-*` 文件 = 7 个引用。

验证：

```bash
# 停止 i2c 设备消费者后干净卸载 i2c_dev（销毁字符设备，强制释放所有 fd）
$ modprobe -r i2c_dev
...
这些引用不会出现在 `/sys/module/nvidia/holders/` 中——内核源码（`kernel/module/sysfs.c`）确认 holders 目录只记录模块加载时的符号依赖（`resolve_symbol()` → `ref_module()` → `add_usage_links()`），不记录运行时 `try_module_get()` 创建的引用。同样不会出现在 `lsof` 中（不是用户态文件描述符），所以通过常规手段完全不可见。
...
内核源码确认：`pci_scan_single_device()` 中 `pci_get_slot()` 检查设备是否已存在，存在则直接返回不触发 probe；`pci_bus_add_devices()` 中 `pci_dev_is_added()` 跳过已添加设备。另有更轻量的替代方案：`echo ADDR > /sys/bus/pci/drivers_probe` 直接调用 `device_attach()` 重新匹配驱动，无需 remove。

### 10. libvirtd hook PATH 缺少系统包

hook 中所有 `fuser -k` 调用静默失败。原因：NixOS 的 libvirtd 模块仅将 qemu、netcat、swtpm 等包加入 systemd service 的 PATH（`virtualisation/libvirtd.nix` 中的 `path` 属性），libvirt 通过 `virCommandAddEnvPassCommon` 将此 PATH 传递给 hook 子进程。`/run/current-system/sw/bin` 不在其中，因此 `environment.systemPackages` 中的 `psmisc`（提供 `fuser`）不可用。这是 NixOS 的已知问题（[nixpkgs#98448](https://github.com/NixOS/nixpkgs/issues/98448)）。
...
`systemctl isolate multi-user.target` 只停止**系统** unit（如 greetd.service），但 `user@UID.service` 在上游 systemd 中显式设置了 `IgnoreOnIsolate=yes`（[systemd issue #1969](https://github.com/systemd/systemd/issues/1969)），因此用户 systemd 实例及其下的所有服务（niri.service、Xwayland 等）不受 isolate 影响。
...
当 prepare/begin hook 失败时，libvirt 会调用 release/end hook（revert hook）。`driver_set_override()` 需要获取 `dev->mutex`（`device_lock`）。如果此时有其他操作（如 `modprobe -r nvidia` 触发的 nvidia remove 回调）持有同一设备的 `dev->mutex`，写 `driver_override` 将阻塞在 `mutex_lock()` 上——内核的 `mutex_lock` 使用 `TASK_UNINTERRUPTIBLE` 状态，因此进程进入不可杀死的 D state，只能重启恢复。
...
1. **i2c_dev + OpenRGB 持有 nvidia 引用**：OpenRGB 打开 7 个 `/dev/i2c-N` 设备文件，`i2cdev_open()` 通过 `try_module_get(adapter->owner)` 对 nvidia 持有 7 个引用。卸载 i2c_dev 销毁字符设备，强制释放所有 fd
2. **用户 systemd 服务持有 DRM fd**：niri 等合成器作为用户 systemd 服务运行，`user@.service` 设置了 `IgnoreOnIsolate=yes`，不受 `systemctl isolate` 影响，必须用 `fuser -k` 显式杀死
...
7. **PCI rescan 不等于驱动重新绑定** — `pci_scan_single_device()` 跳过已存在设备；需先 `remove` 再 `rescan`，或用 `drivers_probe` 直接触发 `device_attach()`
8. **libvirtd hook 有独立的 PATH** — NixOS libvirtd 模块仅设置 qemu/netcat/swtpm 到 service PATH（[nixpkgs#98448](https://github.com/NixOS/nixpkgs/issues/98448)），`|| true` 会静默吞掉 "command not found"
9. **`systemctl isolate` 不影响 `user@.service`** — 上游 systemd 设置 `IgnoreOnIsolate=yes`（[systemd#1969](https://github.com/systemd/systemd/issues/1969)），需用 `fuser` 或 `loginctl` 显式处理
10. **Revert hook 必须幂等** — prepare 失败后 libvirt 仍调用 release hook；`driver_override` 写入需要 `dev->mutex`，若 nvidia remove 回调正持有该锁则 D state
